...
(defn build
  ...
  (Operation. description (util/to-format amount) (util/string->date date)))
...
(defn- compute-balances
  ...
  [ops account]
  {:pos [vector?]}
  (letfn [(sum-last [val arr]
            (util/to-format (+ val (if (empty? arr) 0 (-> arr last :balance)))))
          (compress [a [k v]]
            (conj a {:date k :amount (apply + (map :amount v))}))
          (build-sum [a {:keys [date amount]}]
            (conj a {:date (util/date->string date) :balance (sum-last amount a)}))]
...
(defn- compute-each-balance
  ...
  [ops account]
  {:pos [map?]}
  (let [compress (fn [m {:keys [balance date]}] (assoc m date balance))]
    (reduce compress {} (compute-balances ops account))))
...
(defn new-operation
  ...
  {:pre [ops account description amount date]
   :pos [map?]}
  (update ops account #(conj % (build description amount date))))
...
(defn current-balance
  ...
  (if (contains? ops account)
    {:balance (->> (ops account) (map :amount) (apply +) util/to-format)}))
...
(defn debt-periods
  ...
  {:debts (let [mconj (fn [coll & xs] (apply conj coll (filter map? xs)))
                [head & tail] (drop-while (comp not neg? :balance)
                  ...
                  (if (= balance (:principal head#))
                    ...
                    (mconj tail#
                           (if (contains? head# :end)
                             head#
                             (assoc head# :end (util/previous-day date)))
                           (if (neg? balance)
                             {:start date :principal balance}))))
